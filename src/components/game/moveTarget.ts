...
interface MoveTargetState {
  ...
  // whatever's already locked as combatStore's targetId (set by the same click), not
  // tracked here — this is just "cast once you arrive," not a second target reference.
  pendingSkillCast: boolean;
}

export const moveTarget: MoveTargetState = { point: null, attackTargetId: null, pendingSkillCast: false };
...
export function setSkillMoveTarget(x: number, z: number): void {
  moveTarget.point = new THREE.Vector3(x, 0, z);
  moveTarget.attackTargetId = null;
  moveTarget.pendingSkillCast = true;
}
